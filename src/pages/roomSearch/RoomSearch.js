...
const buttons = [
  ...
  let body = [];
    ...
    reservableRooms.forEach((room) => {
      const { title, roomNumber, floor, totalPrice, images, score } = room;
      let newItem = {
        tableData: [
          title,
          roomNumber,
  ...
  const detailHandler = (roomInfo) => {
    navigate(`/roomDetails/${roomInfo.id}?strength=${strength}`);
  };
      ...
      <div className="search-top">
        ...
        {showResults && (
          <div className="search-bottom">
            <Pagination title={title} body={body} actions={actions} photoes={img} />
          </div>
        )}
      </div>
    </div>
